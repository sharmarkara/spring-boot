...
public class EmployeeDaoImpl implements IEmployeeDao {
	...
	private static final String SQL_FIND_BY_ID = "select * from employee where eid=? ";
	...
	public EmployeeBO findById(Integer eid) {
		...
		try{
			...
			PreparedStatement pstmt = connection.prepareStatement("select * from employee where eid=?");
			pstmt.setInt(1, eid);
			ResultSet resultSet = pstmt.executeQuery();
			while(resultSet.next()) {
	...
	public String deleteById(Integer eid) {
		try {
			...
			System.out.println("*****Affected row is ::"+row);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return "Employee deleted successfylly with "+eid;
	}
